...
class Watcher extends Emitter {
  ...
  constructor(inputPath, options) {
    ...
    this.metaLoadCssInputPaths = options.metaLoadCssInputPaths
      ? options.metaLoadCssInputPaths.map(inputPath => path.resolve(inputPath))
      : [];
  ...
  getIncludedFiles = () => {
    const newWatchedFilesGraph = grapher.parseFile(this.inputPath, {
      loadPaths: this.includePaths,
      extensions: this.includeExtensions,
    });

    let includedFiles = Object.keys(newWatchedFilesGraph.index);

    this.metaLoadCssInputPaths.forEach(inputPath => {
      ...
      includedFiles = [
        ...includedFiles,
        ...Object.keys(newWatchedFilesGraph.index)
      ];
    });
